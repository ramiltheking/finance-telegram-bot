...
class TelegramWebhookController extends Controller
{
    ...
    protected function handleMessage(array $message)
    {
        ...
        if ($text) {
            ...
            if ($text !== null) {
                ...
                $this->tg->sendMessage(
                    $chatId, '❗ Я пока понимаю только текст или голосовые сообщения.'
                );
            }
        }
    }
    ...
    protected function sendStartMessage($chatId, $userId)
    {
        ...
        $this->tg->sendMessage($chatId, "👋 Добро пожаловать!", $keyboard);
    }
    ...
    protected function sendTarifs($chatId)
    {
        ...
        Payment::create(['chat_id' => $chatId, 'inv_id' => $invId, 'amount' => $priceInKZT, 'status' => 'pending']);

        $keyboard = ['inline_keyboard' => [[['text' => "💰 Оплатить {$tariffName} тариф", 'url' => $url]]]];
        $this->tg->sendMessage($chatId, "🎁 Стартовый период — 2 недели.\n\n📦 Далее взимается тарифная помесячная оплата:\n\n💼 <b>{$tariffName}</b> — {$priceInKZT} ₸ в месяц\n\nНажимая оплатить я даю согласие на регулярные списания, на обработку персональных данных и принимаю условия публичной оферты", $keyboard);
    }
